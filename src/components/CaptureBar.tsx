...
export default function CaptureBar({ action }: CaptureBarProps) {
  return (
    <form action={action} className="rounded-3xl border border-slate-800 bg-slate-900/70 p-5 shadow-[0_20px_45px_rgba(15,23,42,0.5)]">
      <div className="flex flex-wrap gap-3">
        <div className="flex-1 min-w-[220px]">
          <label className="text-xs font-semibold uppercase tracking-[0.4em] text-slate-400">Schnell hinzufuegen</label>
          <input
            name="title"
        ...
        <div className="flex flex-col gap-2">
          <label className="text-xs font-semibold uppercase tracking-[0.4em] text-slate-400">Aufwand</label>
          <select name="estimate" className="w-32 rounded-2xl border border-slate-800 bg-slate-950 px-3 py-3 text-sm text-white">
