...
import PomodoroDock, {
  PomodoroScheduledBlock,
} from "../../../components/PomodoroDock";
...
const getGermanyNow = () => {
  ...
  const offsetMatch = (values.timeZoneName ?? "GMT+0").match(
    /GMT([+-]\d{1,2})(?::?(\d{2}))?/
  );
  let offsetMinutes = 0;
  if (offsetMatch) {
    ...
    offsetMinutes =
      hoursValue * 60 + (hoursValue >= 0 ? minutesValue : -minutesValue);
  }
...
const buildWeekDays = (start: Date): WeekDay[] =>
  Array.from({ length: WEEK_LENGTH }, (_, index) => {
    ...
    return {
      ...
      date: day.toLocaleDateString("de-DE", {
        day: "2-digit",
        month: "2-digit",
      }),
      iso,
...
export default async function PlanPage({ searchParams }: PlanPageProps) {
  ...
  const completedPercentage = Math.round(
    (planResult.tasks.filter((task) => task.status === "DONE").length /
      totalTasks) *
      100
  );
  ...
  const plannedTaskIds = new Set(
    planResult.allocations.map((allocation) => allocation.taskId)
  );
  const plannedTasks = planResult.tasks.filter((task) =>
  ...
  const overdueCount = planResult.tasks.filter(
    (task) =>
      task.status !== TaskStatus.DONE && task.dueAt && task.dueAt < today
  ).length;
  const deadlineCount = planResult.tasks.filter((task) =>
    Boolean(task.dueAt)
  ).length;
  ...
  const statusRows = [
    ...
    {
      label: "Im Test",
      value: effectiveStatusCounts.INBOX ?? 0,
  ...
  const nextTasks = planResult.tasks
    ...
    .sort((a, b) =>
      a.dueAt && b.dueAt ? a.dueAt.getTime() - b.dueAt.getTime() : 0
    )
    .slice(0, 3);
  ...
  const calendarMonthLabel = weekStart.toLocaleString("de-DE", {
    month: "long",
    year: "numeric",
  });
  const appointmentCount = weekDays.reduce(
  ...
  const meetingMinutes = weekCalendarBlocks.reduce(
    (sum, block) =>
      sum + Math.round((block.end.getTime() - block.start.getTime()) / 60000),
    0
  );
  const focusMinutes = planResult.allocations.reduce(
    (sum, allocation) => sum + allocation.minutes,
    0
  );
  const openFocusSlots = planResult.slots.filter(
    (slot) => slot.type === "focus" && slot.availableMinutes > 0
  ).length;
  ...
  const plannedEnergy = Math.max(
    1,
    Math.round(
      planResult.tasks.reduce((sum, task) => sum + (task.energy ?? 0), 0) /
        Math.max(1, planResult.tasks.length)
    )
  );
  const capacityHoursValue = Math.max(
    1,
    Math.round((focusMinutes + meetingMinutes) / 60)
  );
  ...
  const topPriorityLabel = topPriorityTitles.length
    ? topPriorityTitles.join(" · ")
    : "Noch keine Prioritäten";
    ...
    <section className="space-y-8">
      <div className="grid gap-6 lg:grid-cols-[1.5fr_0.8fr_0.8fr]">
        <div className="rounded-3xl border border-slate-800 bg-gradient-to-br from-slate-950/70 to-slate-900/80 p-8 shadow-[0_35px_60px_rgba(15,23,42,0.7)]">
          ...
          <div className="mt-4 flex flex-wrap gap-3 text-xs font-semibold uppercase tracking-[0.3em] text-slate-300">
            <span className="rounded-full bg-slate-800/60 px-3 py-1 text-cyan-300">
              {plannedCount} geplant
            </span>
            <span className="rounded-full bg-slate-800/60 px-3 py-1 text-amber-300">
              ...
              <div key={row.label} className="flex items-center gap-3">
                <div className="flex-1">
                  ...
                  <div className="mt-1 h-2 rounded-full bg-slate-800">
                    <div
                      className={`h-full w-full rounded-full bg-gradient-to-r ${row.color}`}
                    />
                  </div>
                </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.2fr_1fr]">
        ...
        <div className="space-y-4 rounded-3xl border border-slate-800 bg-slate-900/60 p-6">
          ...
          <div className="rounded-2xl border border-slate-800/50 bg-slate-950/40 p-4">
            <p className="text-xs font-semibold uppercase tracking-[0.4em] text-slate-400">
              Kapazität: {capacityLabel}
            </p>
            <p className="mt-2 text-sm text-slate-300">
              Meetings blockiert - {meetingHoursLabel}
            </p>
            <p className="text-sm text-slate-300">
              Focus-Fenster - {focusHoursLabel}
            </p>
          </div>
          <div className="rounded-2xl border border-slate-800/50 bg-slate-950/40 p-4">
            <p className="text-xs font-semibold uppercase tracking-[0.4em] text-slate-400">
